...
## 8) نتائج الاختبار المتكامل 2026-08-30 (فرع `integrated-fix` `dd925ad`)

- **1) تصحيح العامل المحاصر** `src/planner.py:442` `allowed() قبل has_work` — وحده: `mean 101973` `worst 77129` `day1 14.0` `escapes 0.25 8/32` **مطابق للأساس** (آمن، يفتح الباب لتخصيص مستقبلي)
  - **dedicated-feeder فوق التصحيح (patch حقيقي داخل zone):** `mean 99894` `worst 77226` `escapes 0.22 7/32` vs `101973` — **لا انهيار** (مقابل `39826` و `28/32` قبل التصحيح)، لكن `-2079` mean — **لا يحسن، يبقى تجريبي**
  - **dynamic-feeder** `ceil(animals/4)`: `mean 101973` مطابق — **محايد**

- **2) ترتيب محاصيل ذكي:** `day0 WHEAT أولاً` + `day≥20 WHEAT/CARROT سريع` `src/planner.py:372` — وحده: `mean 101005` `worst 68495` `day1 14.0` `escapes 0.47 15/32` `field 13.2` vs `101973` — **-968 mean, -8634 worst, هروب أسوأ** — **مرفوض** (العمود الفقري MELON/STRAW يبقى، القمح تكتيكي فقط كما وُثق)

- **الخلاصة المتكاملة:** `fix trapped` وحده آمن ومحايد، `dedicated` أصبح آمناً لكن لا يضيف، `smart crop` مرفوض — **الأساس `101973` يبقى الأفضل**، و `zone+DROP` هو التحسين الحقيقي.
