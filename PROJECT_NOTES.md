...
## 7) مقترح إعادة اختبار على main الجديد (بعد zone) — الأولوية لـ PLAN/حيوانات

> `main` الجديد `f95bacd→ad59a0d` غيّر الأرضية: `escapes 26/32→8/32` و `PASS 3/4→0/4` و `day1 5→14` — أي فكرة فشلت سابقاً بسبب تكلفة هروب أو نقص عمالة قد تتصرف مختلفاً الآن. التالي هو الأعطف لإعادة الاختبار:

1. **`dedicated-feeder` (1 feeder: `hand0` FEED/CARE فقط)** `src/planner.py:334` — سابقاً `6 نبات 32/32` vs `23/32` أساس (هروب 100% vs 72%) — **مرفوض** لكنه PLAN/حيوان مباشر. الآن `escapes 8/32` أساس أقل بـ 18 نقطة، و `zone` وفّر `PASS 0/4` (فائض عمالة)، فتخصيص عامل واحد للتغذية قد لا يحبس نصف الزراعة كما قبل. يُختبر فوق `zone` مباشرة (بدون تغيير `config.py`).

2. **`dynamic-feeder` (`ceil(animals/4)` feeders)** `8097ac9` — سابقاً `6 نبات 28/32` vs `23/32` — أقرب للنجاح من الثابت (28 vs 32). مع قطيع `6+3=9` → `ceil(9/4)=3` feeders، يتماشى مع `HIRE_TARGET 12` و `zone`، قد يقلل `avg 1.17` هروب دون خسارة `day1`.

3. **`WHEAT 25 + SHEEP 9` فوق zone** (ليس في قائمة الفشل لكن مرتبط بـ PLAN) — سابقاً `WHEAT 15→25` + `SHEEP 3→9` أعطى `mean +1.5k` لكن `escapes 26→31/32` — فشل بسبب عمالة. مع `zone` (`escapes 8/32`) قد يتحمل القطيع الأكبر. يُختبر كـ `config.py` فقط (السابق أثبت التعارض `P4V2_FLOOR 8` حُلّ).

> الترتيب: 1→2→3، كل واحد يُختبر منفرداً على `main` الجديد `101973/77129/8-32/14` كأساس، 32 مباراة، نفس مقاييس `mean/worst/escapes/day1`.
